"""FastAPI routes. Auth and storage logic live in auth.py and storage.py."""
...
# 10 MB cap — Lambda payload limit is 6 MB but we read into memory before sending to S3.
_MAX_FILE_SIZE = 10 * 1024 * 1024

# ASCII-only filenames — avoids path traversal and unpredictable S3 key behaviour.
...
# Lambda handler — Mangum wraps ASGI for the Lambda event format.
# ECS uses uvicorn directly via CMD override in the task definition.
handler = Mangum(app)
